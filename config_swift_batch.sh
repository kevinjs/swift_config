...
. ./include_swift_batch.sh

for node in ${NODES};do
	echo "stoping swift on nodes@"${node}
        sshpass -p csdb123cnic ssh stack@${node} swift-init all stop 1>/dev/null 2>&1
done
...
do
	...
	do
		...
		sshpass -p csdb123cnic ssh stack@${node} swift-init all restart 1>/dev/null 2>&1
		sleep 5
	done
done
